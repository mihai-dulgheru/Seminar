...
router
  ...
  .get(async (req, res) => {
    try {
      ...
      const employees = await Employee.findAll({
        attributes:
          simplified && simplified.toLowerCase() === 'true' ? { exclude: ['id', 'createdAt', 'updatedAt'] } : undefined,
        order:
          sortBy && Object.keys(Employee.rawAttributes).includes(sortBy)
            ? [[sortBy, order && (order === 'ASC' || order === 'DESC') ? order : 'ASC']]
            : undefined,
