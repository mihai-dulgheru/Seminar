...
application.get('/', async (request, response, next) => {
  try {
    const result = []
    for (const u of await University.findAll()) {
      const university = {
        name: u.name,
        students: [],
        courses: [],
      ...
      for (const c of await u.getCourses()) {
        university.courses.push({
          key: c.id,
          name: c.name
        })
        for (const s of await c.getStudents()) {
      ...
      for (const s of await u.getStudents()) {
