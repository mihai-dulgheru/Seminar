...
async function getFlight(key) {
    try {
        var flight = {};
        var object = await getObjectFromUrl(`https://data-live.flightradar24.com/clickhandler/?version=1.5&flight=${key}`);
        flight = {
...
async function getFlights() {
    let flights = [];
    let object = await getObjectFromUrl(`https://data-live.flightradar24.com/zones/fcgi/feed.js?faa=1&bounds=49.751%2C42.639%2C17.822%2C32.818&satellite=1&mlat=1&flarm=1&adsb=1&gnd=1&air=1&vehicles=1&estimated=1&maxage=14400&gliders=1&stats=1`);
    for (const key in object) {
        if (typeof object[key] === 'object') {
            let flight = await getFlight(key);
